...
RUN mkdir -p /var/log/supervisor /run/php /var/log/nginx \
...
RUN echo '#!/bin/bash' > /start.sh && \
    echo 'set -e' >> /start.sh && \
    echo 'echo "=== CASHDIGITAL INICIANDO ==="' >> /start.sh && \
    echo 'echo "Verificando archivos..."' >> /start.sh && \
    echo 'ls -la public/index.php' >> /start.sh && \
    echo 'ls -la vendor/autoload.php' >> /start.sh && \
    ...
    echo 'echo "Limpiando caches..."' >> /start.sh && \
    ...
    echo 'echo "Rutas registradas:"' >> /start.sh && \
    echo 'php artisan route:list' >> /start.sh && \
